...
// GetMetricsWithFallback returns metrics, computing on-the-fly when not yet persisted.
// This ensures the frontend gets live metrics (Total Return, Drawdown, etc.) while the run is active,
// and trade metrics (Win Rate, Profit Factor, Best Symbol) as soon as trades are recorded.
func (m *Manager) GetMetricsWithFallback(runID string) (*Metrics, error) {
	metrics, err := LoadMetrics(runID)
	if err == nil {
		return metrics, nil
	}
	...
	if runner, ok := m.GetRunner(runID); ok {
		s := runner.SnapshotState()
		state = &s
	}

	return CalculateMetrics(runID, cfg, state)
}

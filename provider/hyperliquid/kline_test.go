...
func TestGetCandles_StockPerps(t *testing.T) {
	...
	for _, symbol := range symbols {
		...
		if err != nil {
			t.Errorf("Failed to fetch %s: %v", symbol, err)
			continue
		}
...
func TestGetAllMids(t *testing.T) {
	...
	t.Log("=== Cryptocurrency Mid Prices (default dex) ===")
	...
	t.Logf("\n%d cryptocurrency pairs in total", len(mids))
}
...
func TestGetAllMidsXYZ(t *testing.T) {
	...
	t.Log("=== xyz dex Asset Mid Prices (stocks, forex, and commodities) ===")

	for symbol, mid := range mids {
		t.Logf("%s: %s", symbol, mid)
	}

	t.Logf("\n%d xyz dex pairs in total", len(mids))
}
...
func TestGetMeta(t *testing.T) {
	...
	t.Logf("%d assets in total", len(meta.Universe))

...
// walletBalanceCache caches wallet balance responses for 60 seconds
var (
	walletBalanceCache   = make(map[string]walletBalanceCacheEntry)
	walletBalanceCacheMu sync.RWMutex
	walletBalanceTTL     = 60 * time.Second
)

type walletBalanceCacheEntry struct {
	data    *wallet.AccountBalanceResult
	expires time.Time
}
...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		api.GET("/market/tickers", s.handleMarketTickers)
		api.GET("/market/global", s.handleMarketGlobal)
		api.GET("/market/coins", s.handleMarketCoins)
...
func (s *Server) handleWalletBalances(c *gin.Context) {
	...
	isSolana := wallet.IsSolanaAddress(address)
	isEVM := wallet.IsEVMAddress(address)

	if !isEVM && !isSolana {
		SafeBadRequest(c, "unsupported address format")
		return
	}
...
// handleWalletAnalyze streams AI analysis of wallet holdings via SSE
func (s *Server) handleWalletAnalyze(c *gin.Context) {
	address := strings.TrimSpace(c.Param("address"))
	if address == "" {
		SafeBadRequest(c, "address is required")
		return
	...
	cfg := config.Get()
	if cgKey := cfg.CoinGeckoAPIKey; cgKey != "" {
		logger.Infof("Enriching wallet balances with CoinGecko price stats for %s", address)
		wallet.EnrichWithCoinGecko(result, cgKey)
	} else {
		logger.Infof("Skipping CoinGecko enrichment: COINGECKO_API_KEY not set")
	}

	// Grok API key
	apiKey := cfg.GetSystemAPIKey("grok")
	if apiKey == "" {
		SafeInternalError(c, "AI analysis service", fmt.Errorf("GROK_API_KEY not configured"))
		return
	}

	// Create Grok client
	grokClient := mcp.NewGrokClientWithOptions()
	grokClient.SetAPIKey(apiKey, "", cfg.GetSystemModelName("grok"))
	grokClient.SetTimeout(120 * time.Second)

	// Run AI analysis
	lang := strings.TrimSpace(strings.ToLower(c.Query("lang")))
	if lang != "zh" {
		lang = "en"
	}

	grokCaller := grokClient.(mcp.ResponsesCaller)
	model := cfg.GetSystemModelName("grok")
	if model == "" {
		model = mcp.DefaultGrokModel
	}
	analysis, err := wallet.AnalyzeWalletGrok(grokCaller, model, result, address, lang)
	if err != nil {
		SafeInternalError(c, "AI wallet analysis", err)
		return
	}
	...
	// Stream response in chunks (by line) for typewriter effect
...
// chatRequest is the JSON body for POST /wallet/:address/chat
type chatRequest struct {
	Message         string               `json:"message"`
	InitialAnalysis string               `json:"initialAnalysis"`
	History         []wallet.ChatMessage `json:"history"`
}

// handleWalletChat handles follow-up questions to the AI wallet analysis
func (s *Server) handleWalletChat(c *gin.Context) {
	address := strings.TrimSpace(c.Param("address"))
	if address == "" {
		SafeBadRequest(c, "address is required")
		return
	...
	if msg == "" {
		SafeBadRequest(c, "message is required")
		return
	}
	if body.InitialAnalysis == "" {
		SafeBadRequest(c, "initialAnalysis is required")
		return
	}
	...
	apiKey := cfg.GetSystemAPIKey("grok")
	if apiKey == "" {
		SafeInternalError(c, "AI chat service", fmt.Errorf("GROK_API_KEY not configured"))
		return
	}
	...
	rawReply, err := wallet.ChatWithWalletGrok(grokCaller, model, result, address, body.InitialAnalysis, body.History, msg, lang)
	if err != nil {
		SafeInternalError(c, "AI wallet chat", err)
		return
	}

	reply, swapIntent := wallet.ParseSwapIntent(rawReply)
	resp := gin.H{"reply": reply}
	if swapIntent != nil {
		resp["swapIntent"] = swapIntent
	}
	c.JSON(http.StatusOK, resp)
}

// generalChatRequest is the JSON body for POST /chat
type generalChatRequest struct {
	Message string               `json:"message"`
	History []wallet.ChatMessage `json:"history"`
}

// handleGeneralChat handles general crypto questions without wallet context
func (s *Server) handleGeneralChat(c *gin.Context) {
	var body generalChatRequest
	if err := c.ShouldBindJSON(&body); err != nil {
		SafeBadRequest(c, "invalid request body")
		return
	}
	...
	reply, err := wallet.GeneralChatGrok(grokCaller, model, body.History, msg, lang)
	if err != nil {
		SafeInternalError(c, "AI general chat", err)
		return
	}

	cleanReply, swapIntent := wallet.ParseSwapIntent(reply)
	resp := gin.H{"reply": cleanReply}
	if swapIntent != nil {
		resp["swapIntent"] = swapIntent
	}
	c.JSON(http.StatusOK, resp)
...
type SafeModelConfig struct {
	...
	HasSystemKey    bool   `json:"hasSystemKey"`    // True if system-level API key is configured
}
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	// Ensure AI model exists in DB when using system API key (e.g. CLAUDE_API_KEY from .env)
	// This matches the logic in CreateDebate - allows users to use providers from default list without manually adding in Config
	cfg := config.Get()
	_, modelErr := s.store.AIModel().Get(userID, req.AIModelID)
	if modelErr != nil {
		provider := strings.ToLower(strings.TrimSpace(req.AIModelID))
		if (provider == "claude" || provider == "deepseek" || provider == "qwen" || provider == "openai" || provider == "gemini" || provider == "grok" || provider == "kimi") && cfg.GetSystemAPIKey(provider) != "" {
			if ensureErr := s.store.AIModel().Update(userID, req.AIModelID, true, "", "", ""); ensureErr != nil {
				logger.Warnf("Failed to ensure AI model %s: %v", req.AIModelID, ensureErr)
			} else {
				logger.Infof("✓ Ensured AI model %s (using system API key) for trader creation", req.AIModelID)
			}
		}
	}
...
// handleMarketTickers streams live Binance price tickers via Server-Sent Events.
// The backend maintains a single persistent WebSocket to Binance and fans out
// updates to all connected SSE clients, avoiding per-browser WebSocket overhead.
func (s *Server) handleMarketTickers(c *gin.Context) {
	c.Header("Content-Type", "text/event-stream")
	c.Header("Cache-Control", "no-cache")
	c.Header("Connection", "keep-alive")
	c.Header("X-Accel-Buffering", "no")

	// Send current snapshot immediately so the ticker bar populates on load
...
// handleMarketGlobal serves cached global market stats + fear & greed index.
func (s *Server) handleMarketGlobal(c *gin.Context) {
	data, ok := market.GetGlobalData()
	if !ok {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "data not ready yet"})
		return
	}
...
// handleMarketCoins serves the cached top-20 coins by market cap.
func (s *Server) handleMarketCoins(c *gin.Context) {
	data, ok := market.GetTopCoins()
	if !ok {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "data not ready yet"})
		return
	}
...
// handleMarketTrending serves the cached trending coins list.
func (s *Server) handleMarketTrending(c *gin.Context) {
	data, ok := market.GetTrending()
	if !ok {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "data not ready yet"})
		return
	}
...
// handleMarketGainers serves the cached top gainers list.
func (s *Server) handleMarketGainers(c *gin.Context) {
	data, ok := market.GetTopGainers()
	if !ok {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "data not ready yet"})
		return
	}
...
// handleMarketChart serves the cached 7-day market cap + BTC price history.
func (s *Server) handleMarketChart(c *gin.Context) {
	data, ok := market.GetChartData()
	if !ok {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "data not ready yet"})
		return
	}
...
func (s *Server) handleGetSupportedModels(c *gin.Context) {
	...
	supportedModels := []map[string]interface{}{
		{"id": "deepseek", "name": "DeepSeek", "provider": "deepseek", "defaultModel": "deepseek-chat", "hasSystemKey": cfg.GetSystemAPIKey("deepseek") != ""},
		{"id": "qwen", "name": "Qwen", "provider": "qwen", "defaultModel": "qwen3-max", "hasSystemKey": cfg.GetSystemAPIKey("qwen") != ""},
		{"id": "openai", "name": "OpenAI", "provider": "openai", "defaultModel": "gpt-5.1", "hasSystemKey": cfg.GetSystemAPIKey("openai") != ""},
		{"id": "claude", "name": "Claude", "provider": "claude", "defaultModel": "claude-opus-4-6", "hasSystemKey": cfg.GetSystemAPIKey("claude") != ""},
		{"id": "gemini", "name": "Google Gemini", "provider": "gemini", "defaultModel": "gemini-3-pro-preview", "hasSystemKey": cfg.GetSystemAPIKey("gemini") != ""},
		{"id": "grok", "name": "Grok (xAI)", "provider": "grok", "defaultModel": "grok-3-latest", "hasSystemKey": cfg.GetSystemAPIKey("grok") != ""},
		{"id": "kimi", "name": "Kimi (Moonshot)", "provider": "kimi", "defaultModel": "moonshot-v1-auto", "hasSystemKey": cfg.GetSystemAPIKey("kimi") != ""},
	}

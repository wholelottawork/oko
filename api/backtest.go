...
func (s *Server) handleBacktestStart(c *gin.Context) {
	...
	if cfg.StrategyID != "" {
		...
		// If no timeframes provided, use strategy's timeframes
		if len(cfg.Timeframes) == 0 && len(strategyConfig.Indicators.Klines.SelectedTimeframes) > 0 {
			cfg.Timeframes = strategyConfig.Indicators.Klines.SelectedTimeframes
			if cfg.DecisionTimeframe == "" && strategyConfig.Indicators.Klines.PrimaryTimeframe != "" {
...
func (s *Server) handleBacktestMetrics(c *gin.Context) {
	...
	metrics, err := s.backtestManager.GetMetricsWithFallback(runID)
	if err != nil {
...
func (s *Server) handleBacktestConfig(c *gin.Context) {
	...
	// Sanitize: strip API key for client
	safe := *cfg
	safe.AICfg.APIKey = ""
...
func (s *Server) hydrateBacktestAIConfig(cfg *backtest.BacktestConfig) error {
	...
	if modelID != "" {
		...
		if err != nil {
			// Fallback: user may have selected from default list (e.g. "deepseek") without DB record
			model = s.fallbackModelFromID(modelID)
			if model == nil {
				return fmt.Errorf("Failed to load AI model: %w", err)
			}
			cfg.AIModelID = modelID
			logger.Infof("📊 Using fallback model config for %s (system key)", modelID)
		}
	} else {
	...
	userKey := strings.TrimSpace(string(model.APIKey))
	systemKey := config.Get().GetSystemAPIKey(strings.ToLower(strings.TrimSpace(model.Provider)))
	hasSystemKey := systemKey != ""

	if !model.Enabled && !(hasSystemKey && userKey == "") {
	...
	if apiKey == "" {
		apiKey = systemKey
		if apiKey == "" {
			return fmt.Errorf("AI model %s has no API key configured", model.Name)
		}
		logger.Infof("Using system API key for provider %s", model.Provider)
	}
	...
	if modelName == "" {
		modelName = config.Get().GetSystemModelName(provider)
	}
	if cfg.AICfg.Model == "" {
		cfg.AICfg.Model = modelName
	}
	cfg.AICfg.Model = strings.TrimSpace(cfg.AICfg.Model)
	// DeepSeek API requires model name like "deepseek-chat", not just "deepseek"
	if cfg.AICfg.Provider == "deepseek" && (cfg.AICfg.Model == "" || cfg.AICfg.Model == "deepseek") {
		cfg.AICfg.Model = config.Get().GetSystemModelName("deepseek")
	}
...
// fallbackModelFromID returns a model config for known IDs when no DB record exists.
// Used when user selects from default list (e.g. "deepseek") without having enabled it.
func (s *Server) fallbackModelFromID(modelID string) *store.AIModel {
	cfg := config.Get()
	modelID = strings.ToLower(strings.TrimSpace(modelID))
	systemKey := cfg.GetSystemAPIKey(modelID)
	if systemKey == "" {
		return nil
	}
	// Map model ID to display name (model name comes from config.GetSystemModelName)
	var name string
	switch modelID {
	case "deepseek":
		name = "DeepSeek AI"

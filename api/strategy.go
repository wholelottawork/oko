...
func (s *Server) runRealAITest(userID, modelID, systemPrompt, userPrompt string) (string, error) {
	...
	// API key: fall back to system .env when user has not configured their own
	apiKey := strings.TrimSpace(string(model.APIKey))
	if apiKey == "" {
		apiKey = config.Get().GetSystemAPIKey(provider)
	}
	if apiKey == "" {
		return "", fmt.Errorf("AI model %s is missing API Key (configure in Config or set %s_API_KEY in .env)", model.Name, strings.ToUpper(provider))
	}
	// Model name: treat bare provider names (e.g. "deepseek") as invalid
	...
	case "qwen":
		...
		aiClient.SetAPIKey(apiKey, model.CustomAPIURL, modelName)
	case "deepseek":

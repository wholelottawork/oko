...
var knownProviders = map[string]bool{
	...
	"gemini": true, "grok": true, "kimi": true,
}
...
func (h *DebateHandler) HandleCreateDebate(c *gin.Context) {
	...
	for i, p := range req.Participants {
		modelID := strings.TrimSpace(p.AIModelID)
		if modelID == "" {
			continue
		}
		aiModel, err := h.aiModelStore.GetByID(modelID)
		if err != nil {
			// Allow default provider IDs when system has API key (from /models default list)
			if knownProviders[modelID] && config.Get().GetSystemAPIKey(modelID) != "" {
				if err := h.aiModelStore.Update(userID, modelID, true, "", "", ""); err != nil {
					logger.Warnf("Failed to ensure AI model %s: %v", modelID, err)
					continue
				}
				// Model created with ID userID_provider
				realID := fmt.Sprintf("%s_%s", userID, modelID)
				aiModel, err = h.aiModelStore.GetByID(realID)
				if err != nil {
					logger.Warnf("AI model not found after ensure: %s", realID)
					continue
				}
			} else {
				logger.Warnf("AI model not found: %s", modelID)
				continue
			}
		}
		if aiModel.UserID != userID && aiModel.UserID != "default" {
			logger.Warnf("AI model %s does not belong to user", modelID)
			continue
		}
	...
	if fullSession == nil || len(fullSession.Participants) < 2 {
		_ = h.debateStore.DeleteSession(session.ID)
		c.JSON(http.StatusBadRequest, gin.H{"error": "at least 2 valid AI models required (configure models in Config or ensure system has API keys)"})
		return
	}

...
// resolveModelName returns user's CustomModelName if set and valid, otherwise server's .env model name.
// Treat bare provider names (e.g. "deepseek", "claude") as invalid - they cause "Model Not Exist".
func resolveModelName(userModel, provider string) string {
	userModel = strings.TrimSpace(userModel)
	if userModel != "" && userModel != provider {
		return userModel
	}
	return config.Get().GetSystemModelName(provider)
}
...
func (tm *TraderManager) LoadTradersFromStore(st *store.Store) error {
	...
	for _, traderCfg := range allTraders {
		...
		userKey := strings.TrimSpace(string(aiModelCfg.APIKey))
		hasSystemKey := config.Get().GetSystemAPIKey(strings.ToLower(strings.TrimSpace(aiModelCfg.Provider))) != ""
		if !aiModelCfg.Enabled && !(hasSystemKey && userKey == "") {
			logger.Infof("⚠️  AI model %s for trader %s is not enabled, skipping", traderCfg.AIModelID, traderCfg.Name)
			continue
		}
...
func (tm *TraderManager) addTraderFromStore(traderCfg *store.Trader, aiModelCfg *store.AIModel, exchangeCfg *store.Exchange, st *store.Store) error {
	...
	traderConfig := trader.AutoTraderConfig{
		...
		CustomModelName:       resolveModelName(aiModelCfg.CustomModelName, aiModelCfg.Provider),
	...
	// Fall back to system key when user has not configured their own
	userKey := strings.TrimSpace(string(aiModelCfg.APIKey))
	if userKey == "" {
		userKey = config.Get().GetSystemAPIKey(strings.ToLower(strings.TrimSpace(aiModelCfg.Provider)))
		if userKey != "" {
			logger.Infof("Using system API key for provider %s", aiModelCfg.Provider)
		}
	}
	switch aiModelCfg.Provider {
	case "qwen":
		traderConfig.QwenKey = userKey
	case "deepseek":
		traderConfig.DeepSeekKey = userKey
	default:
		// For other providers (grok, openai, claude, gemini, kimi, etc.), use CustomAPIKey
		traderConfig.CustomAPIKey = userKey
	}

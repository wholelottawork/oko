...
/**
 *  PR #669 Test: Prevent null tokens from causing unauthorized API calls
 *
 *  Problem: When the user is not logged in (user/token is null), SWR will still use the empty key to initiate API requests.
 *  Fix: Add `user && token` check in SWR key, return null when not logged in, prevent API calls
 */

describe('API Guard Logic (PR #669)', () => {
  /**
   *   Test SWR key generation logic
   *  Core fix: key must contain user && token check
   */
  ...
  /**
   *   Test conditional logic for different API endpoints
   *   All endpoints that require authentication should check user && token
   */
  ...
  /**
   *   Test conditional logic for the EquityChart component
   *  PR #669 also fixes the same issue in EquityChart
   */
  ...
  describe('edge cases', () => {
    ...
    it('should handle numeric traderId', () => {
      const user = { id: '1', email: 'test@example.com' }
      const token = 'valid-token'
      const traderId = 123 // numbers instead of strings
      const currentPage = 'trader'
  ...
  /**
   *   Test the logic flow to prevent API calls
   */
  describe('API call prevention flow', () => {
    ...
    it('should simulate SWR behavior with null key', () => {
      // SWR will not initiate a request when key is null
      const key = null

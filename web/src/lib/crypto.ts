...
export interface EncryptedPayload {
  ...
  iv: string // 12 bytes
  ciphertext: string // AES-GCM output (including tag)
  aad?: string // Optional: additional authentication data
  kid?: string // Optional: Server public key identification
  ts?: number // Optional: unix seconds for replay protection
}
...
export class CryptoService {
  ...
  static async fetchCryptoConfig(): Promise<CryptoConfig> {
    const response = await httpClient.get(`${API_BASE}/crypto/config`)
    if (!response.success) {
      throw new Error(`Failed to fetch crypto config: ${response.message}`)
    }
    this._transportEncryption = response.data.transport_encryption
  ...
  private static async importPublicKey(pem: string): Promise<CryptoKey> {
    ...
    const pemContents = pem
      ...
      .replace(/\s+/g, '') // Remove all whitespace characters (including newlines, spaces, etc.)
  ...
  static async encryptSensitiveData(
    ...
    // 3. Prepare AAD (Additional Authentication Data)
    ...
    // 6. Encrypt AES keys using RSA-OAEP
...
// Generate obfuscated strings (for clipboard obfuscation)

...
export function CompetitionPage() {
  ...
  const { data: competition } = useSWR<CompetitionData>(
    ...
    api.getCompetition,
    {
      refreshInterval: 15000, // Refresh in 15 seconds (competition data does not need to be updated too frequently)
  ...
  const handleTraderClick = async (traderId: string) => {
    try {
      ...
      // For non-logged-in users, detailed configuration is not displayed. This is normal behavior.
      // The competition page is mainly used to view the rankings and basic information
    }
  }
  ...
  // If there is data returned but no trader, empty status is displayed
            ...
            <div className="flex items-center gap-3 md:gap-4">
              <div>
                ...
                >
                  ...
                  <span
                    className="text-xs font-normal px-2 py-1 rounded competition-badge"
                  >
      ...
      <div className="w-full px-4 md:px-8 space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-0">
          ...
          <div className="text-left md:text-right w-full md:w-auto">
            ...
            <div
              className="text-base md:text-lg font-bold competition-leader-name"
            >
              {leader?.trader_name}
            </div>
            <div
              ...
              style={{
                color: (leader?.total_pnl ?? 0) >= 0 ? 'var(--binance-green)' : 'var(--binance-red)',
              }}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <div
                className="text-xs px-2 py-1 rounded competition-badge-live"
              >
                {t('live', language)}
              </div>
            </div>
              ...
              {sortedTraders.map((trader, index) => {
                ...
                return (
                  <div
                    ...
                    className={`rounded p-3 transition-all duration-300 hover:translate-y-[-1px] cursor-pointer hover:shadow-lg ${isLeader ? 'competition-leader-card' : ''}`}
                    style={!isLeader ? {
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                            index === 0 ? 'competition-rank-1' :
                            index === 1 ? 'competition-rank-2' :
                            index === 2 ? 'competition-rank-3' : ''
                          }`}
                          style={index >= 3 ? {
                  ...
                  <div
                    ...
                    style={
                      isWinning
                        ? {
                          background:
                            'linear-gradient(135deg, var(--binance-green-bg) 0%, var(--binance-green-bg) 100%)',
                          border: '2px solid rgba(14, 203, 129, 0.3)',
                          boxShadow: '0 3px 15px var(--binance-green-bg)',
                        }
                        : {
                          background: 'var(--surface-primary)',
                          border: '1px solid var(--surface-tertiary)',
                  ...
                  >
                    <div className="text-center">
                      ...
                      <div
                        ...
                        style={{
                          color:
                            (trader.total_pnl ?? 0) >= 0 ? 'var(--binance-green)' : 'var(--binance-red)',
                        }}
                      >
                        {trader.total_pnl_pct != null &&
                          !isNaN(trader.total_pnl_pct)

              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>
                    ...
                    {'Margin'}: ${pos.margin_used.toFixed(2)}
                  </div>
                </div>
              </div>
...
export function BacktestPage() {
  ...
  const handleStart = async (event: FormEvent) => {
    ...
    try {
      ...
      const errMsg = error instanceof Error && error.message ? error.message : tr('toasts.startFailed')
  ...
  const handleControl = async (action: 'pause' | 'resume' | 'stop') => {
    ...
    try {
      ...
      const errMsg = error instanceof Error && error.message ? error.message : tr('toasts.actionFailed')
      setToast({ text: errMsg, tone: 'error' })
    }
  }
  ...
  const handleRerun = async (runId: string) => {
    try {
      ...
      setWizardStep(1)
      setToast({ text: 'Config loaded, cache enabled for re-run', tone: 'success' })
    } catch (err) {
      setToast({ text: err instanceof Error && err.message ? err.message : 'Failed to load config', tone: 'error' })
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errMsg = error instanceof Error && error.message ? error.message : tr('toasts.deleteFailed')
      setToast({ text: errMsg, tone: 'error' })
      await refreshRuns()
    }
  }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const errMsg = error instanceof Error && error.message ? error.message : tr('toasts.exportFailed')
      setToast({ text: errMsg, tone: 'error' })
    }
  }
                  ...
                  {wizardStep === 1 && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          <option value="">{'No saved strategy'}</option>
                          {strategies?.map((s) => (

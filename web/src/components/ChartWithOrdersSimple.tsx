...
export function ChartWithOrdersSimple({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // Get K-line data from our own service
        const limit = 100
  ...
  return (
    <div className="relative" style={{ background: 'var(--surface-primary)', borderRadius: '8px', overflow: 'hidden', minHeight: height }}>
      {/* Chart header */}
        ...
        {error ? (
          ...
          <>
            <div className="p-4 rounded" style={{ background: 'var(--surface-secondary)', border: '1px solid var(--surface-tertiary)' }}>
              <div className="text-sm mb-2" style={{ color: 'var(--text-secondary)' }}>K</div>

...
interface ChartTabsProps {
  traderId: string
  selectedSymbol?: string // Currency selected from outside
  updateKey?: number // Forced update key
...
const MARKET_CONFIG = {
  hyperliquid: { exchange: 'hyperliquid', defaultSymbol: 'BTC', icon: '🔷', label: 'HL', color: 'cyan', hasDropdown: true },
  crypto: { exchange: 'binance', defaultSymbol: 'BTCUSDT', icon: '₿', label: 'Crypto', color: 'accent', hasDropdown: false },
  stocks: { exchange: 'alpaca', defaultSymbol: 'AAPL', icon: '📈', label: 'Stocks', color: 'green', hasDropdown: false },
  forex: { exchange: 'forex', defaultSymbol: 'EUR/USD', icon: '💱', label: 'Forex', color: 'blue', hasDropdown: false },
  metals: { exchange: 'metals', defaultSymbol: 'XAU/USD', icon: '🥇', label: 'Metals', color: 'accent', hasDropdown: false },
}
...
// Infer market type based on exchange ID
function getMarketTypeFromExchange(exchangeId: string | undefined): MarketType {
  ...
  // Other exchanges use crypto type by default
  return 'crypto'
}
...
export function ChartTabs({ traderId, selectedSymbol, updateKey, exchangeId }: ChartTabsProps) {
  ...
  // Automatically switch market types when the exchange ID changes
  useEffect(() => {
    const newMarketType = getMarketTypeFromExchange(exchangeId)
    setMarketType(newMarketType)
  }, [exchangeId])

  // Determine exchange based on market type
  const marketConfig = MARKET_CONFIG[marketType]
  // Prioritize the passed exchangeId (when not hyperliquid)
  ...
  // Update default symbols when switching market types
  ...
  // When selecting a currency from outside, automatically switch to the K-line chart
  useEffect(() => {
    if (selectedSymbol) {
      console.log('[ChartTabs] :', selectedSymbol, 'updateKey:', updateKey)
      setChartSymbol(selectedSymbol)
      setActiveTab('kline')
    }
  }, [selectedSymbol, updateKey])

  // Handling manually entered symbols
  const handleSymbolSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (symbolInput.trim()) {
      let symbol = symbolInput.trim().toUpperCase()
      // Cryptocurrencies automatically add the USDT suffix

...
// Supported exchange list (contract format)
const EXCHANGES = [
...
function TradingViewChartComponent({
  ...
  // When the external defaultSymbol changes, update the internal symbol
  useEffect(() => {
    if (defaultSymbol && defaultSymbol !== symbol) {
      // console.log('[TradingViewChart] Update currency:', defaultSymbol)
      setSymbol(defaultSymbol)
    }
  }, [defaultSymbol])

  // When the external incoming defaultExchange changes, update the internal exchange
  useEffect(() => {
    if (defaultExchange && defaultExchange !== exchange) {
      const normalizedExchange = defaultExchange.toUpperCase()
      // console.log('[TradingViewChart] Update exchange:', normalizedExchange)
  ...
  // Get the complete trading pair symbol (Contract format: BINANCE:BTCUSDT.P)
  const getFullSymbol = () => {
  ...
  // Handling custom trading pair inputs
  const handleCustomSymbolSubmit = () => {
    if (customSymbol.trim()) {
      let sym = customSymbol.trim().toUpperCase()
      // If there is no USDT suffix, it will be added automatically.
  ...
  return (
    <div
      ...
      style={isFullscreen ? { background: 'var(--surface-primary)' } : undefined}
    >
      {/* Header */}
      <div
        className="flex flex-wrap items-center gap-2 p-3 sm:p-4"
        style={{ borderBottom: embedded ? 'none' : '1px solid var(--surface-tertiary)' }}
      >
        {!embedded && (
        ...
        <div className={`flex flex-wrap items-center gap-2 ${embedded ? '' : 'ml-auto'}`}>
              ...
              >
                {EXCHANGES.map((ex) => (
                  <button
                    ...
                    style={{
                      color: exchange === ex.id ? 'var(--accent-primary)' : 'var(--text-primary)',
                      background:
                        exchange === ex.id
          ...
          <div className="relative">
            ...
            {showSymbolDropdown && (
              ...
              >
                {/* Custom Input */}
                <div className="px-3 pb-2" style={{ borderBottom: '1px solid var(--surface-tertiary)' }}>
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    {POPULAR_SYMBOLS.map((sym) => (
                      <button
                        ...
                        style={{
                          color: symbol === sym ? 'var(--accent-primary)' : 'var(--text-primary)',
                          background:
                            symbol === sym
                              ? 'var(--accent-primary-bg)'
                              : 'rgba(43, 49, 57, 0.3)',
          ...
          >
            {INTERVALS.map((int) => (
              <button
                ...
                style={{
                  background: timeInterval === int.id ? 'var(--accent-primary)' : 'transparent',
                  color: timeInterval === int.id ? '#0B0E11' : 'var(--text-secondary)',
                }}
              >
                {int.label}
              </button>

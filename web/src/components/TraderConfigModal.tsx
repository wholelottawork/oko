...
const inputClassName =
  'trader-config-input w-full px-3 py-2 rounded focus:outline-none'

import type { TraderConfigData } from '../types'

// Form internal status type
interface FormState {
  trader_id?: string
  trader_name: string
  ai_model: string
...
export function TraderConfigModal({
    ...
    const fetchStrategies = async () => {
      try {
        ...
        if (result.success && result.data?.strategies) {
          ...
          // If no strategy is selected, the activated strategy is selected by default
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Include initial_balance only in edit mode
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#2B3139] bg-gradient-to-r from-[#1E2329] to-[#252B35] sticky top-0 z-10 rounded-t-xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[var(--accent-primary)] to-[var(--accent-primary-active)] flex items-center justify-center text-[var(--header-btn-text)]">
              {isEditMode ? (
        ...
        >
          ...
          <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-[#EAECEF] mb-5 flex items-center gap-2">
              <span className="text-[var(--accent-primary)]">1</span> {t('basicConfig', language)}
            </h3>
                  ...
                  {formData.exchange_id && (() => {
                    ...
                    return (
                      <a
                        ...
                        className="mt-2 inline-flex items-center gap-1.5 text-xs text-[#848E9C] hover:text-[var(--accent-primary)] transition-colors"
                      >
                        <UserPlus className="w-3.5 h-3.5" />
                        <span>{t('noExchangeAccount', language)}</span>
                        {regLink.hasReferral && (
                          <span className="px-1.5 py-0.5 bg-[var(--accent-primary)]/10 text-[var(--accent-primary)] rounded text-[10px]">
                            {t('discount', language)}
                          </span>
                        )}
          ...
          <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-[#EAECEF] mb-5 flex items-center gap-2">
              <span className="text-[var(--accent-primary)]">2</span> {t('selectTradingStrategy', language)}
              ...
              {selectedStrategy && (
                <div className="mt-3 p-4 bg-[#1E2329] border border-[#2B3139] rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-[#848E9C]">
                    <div>
                      {t('coinSource', language)}: {selectedStrategy.config.coin_source.source_type === 'static' ? '' :
          ...
          <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-[#EAECEF] mb-5 flex items-center gap-2">
              <span className="text-[var(--accent-primary)]">3</span> {t('tradingParams', language)}
            </h3>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1 text-xs bg-[var(--accent-primary)] text-black rounded hover:bg-[var(--accent-primary-active)] transition-colors disabled:bg-[#848E9C] disabled:cursor-not-allowed"
                    >
        ...
        <div className="flex justify-end gap-3 p-6 border-t border-[#2B3139] bg-gradient-to-r from-[#1E2329] to-[#252B35] sticky bottom-0 z-10 rounded-b-xl">
          ...
          {onSave && (
            <button
              ...
              className="px-8 py-3 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-primary-active)] text-black rounded-lg hover:from-[var(--accent-primary-active)] hover:to-[#1D4ED8] transition-all duration-200 disabled:bg-[#848E9C] disabled:cursor-not-allowed font-medium shadow-lg"
            >

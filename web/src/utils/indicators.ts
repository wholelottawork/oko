// Technical indicator calculation tool
...
// Calculate EMA from array of values ​​(helper function)
...
export function calculateRSI(data: Kline[], period = 14): Array<{ time: number; value: number }> {
  ...
  // Calculate the initial average price increase and decrease
  let avgGain = 0
  let avgLoss = 0
  for (let i = 0; i < period; i++) {
    if (changes[i] > 0) {

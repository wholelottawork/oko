...
function ConverterWidget({ coins }: { coins: CoinRow[] }) {
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="mt-1 text-xs font-semibold" style={{ color: 'var(--text-tertiary)' }}>
                    {fearGreed ? fearGreed.classification : ('Fear & Greed')}
                  </div>
                </div>
              </div>
            </div>
              ...
              >
                ...
                <div className="divide-y" style={{ borderColor: 'var(--panel-border)' }}>
                  {(() => {
                    ...
                    const rows = [
                      { label: 'BTC Price', value: fmtPrice(btc.price), sub: `${btc.change24h >= 0 ? '+' : ''}${btc.change24h.toFixed(2)}%`, subColor: pctColor(btc.change24h) },
                      { label: 'ETH Price', value: eth ? fmtPrice(eth.price) : '—', sub: eth ? `${eth.change24h >= 0 ? '+' : ''}${eth.change24h.toFixed(2)}%` : '', subColor: eth ? pctColor(eth.change24h) : 'inherit' },
                      { label: 'BTC Market Cap', value: fmt(btc.marketCap), sub: null, subColor: '' },
                      { label: 'BTC 24h Volume', value: fmt(btc.volume), sub: null, subColor: '' },
                      { label: 'Total Market Cap', value: fmt(totalMcap), sub: global ? `${global.marketCapChange >= 0 ? '+' : ''}${global.marketCapChange?.toFixed(2)}%` : null, subColor: global ? pctColor(global.marketCapChange) : '' },
                      { label: 'BTC Dominance', value: global ? `${global.btcDominance.toFixed(1)}%` : '—', sub: null, subColor: '' },
                      { label: 'Total Volume 24h', value: global ? fmt(global.totalVolume) : '—', sub: null, subColor: '' },
                    ]
                    ...
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--panel-border)' }}>
                        {[
                          ...
                          { label: 'Name', align: 'left', hide: false },
                          { label: 'Price', align: 'right', hide: false },
                          { label: '24h %', align: 'right', hide: false },
                          { label: '7d %', align: 'right', hide: true },
                          { label: 'Market Cap', align: 'right', hide: true },
                          { label: 'Volume (24h)', align: 'right', hide: true },
                          { label: '7d Trend', align: 'center', hide: true },
                        ].map((h, i) => (
                          <th
                            key={i}

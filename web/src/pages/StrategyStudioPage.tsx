...
export function StrategyStudioPage() {
  ...
  const fetchAiModels = useCallback(async () => {
    if (!token) return
    try {
      const response = await httpClient.get(`${API_BASE}/api/models`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (response.success) {
        const data = response.data
        // The backend returns an array, not { models: [] }
  ...
  const fetchStrategies = useCallback(async () => {
    if (!token) return
    try {
      const response = await httpClient.get(`${API_BASE}/api/strategies`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (!response.success) throw new Error('Failed to fetch strategies')
      const data = response.data
  ...
  const handleCreateStrategy = async () => {
    if (!token) return
    try {
      const configResponse = await httpClient.get(
        `${API_BASE}/api/strategies/default-config?lang=${language}`,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      const defaultConfig = configResponse.data
      ...
      if (!response.success) throw new Error('Failed to create strategy')
  ...
  const handleDeleteStrategy = async (id: string) => {
    ...
    try {
      const response = await httpClient.delete(
        `${API_BASE}/api/strategies/${id}`,
        {
          Authorization: `Bearer ${token}`,
        }
      )
      if (!response.success) throw new Error('Failed to delete strategy')
  ...
  const handleDuplicateStrategy = async (id: string) => {
    ...
    try {
      ...
      if (!response.success) throw new Error('Failed to duplicate strategy')
      await fetchStrategies()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unknown error')
    }
  ...
  const handleActivateStrategy = async (id: string) => {
    ...
    try {
      ...
      if (!response.success) throw new Error('Failed to activate strategy')
      await fetchStrategies()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unknown error')
    }
  ...
  const handleImportStrategy = async (
    ...
    try {
      ...
      if (!response.success) throw new Error('Failed to import strategy')
  ...
  const handleSaveStrategy = async () => {
    ...
    try {
      ...
      if (!response.success) throw new Error('Failed to save strategy')
      setHasChanges(false)
  ...
  const fetchPromptPreview = async () => {
    ...
    try {
      ...
      if (!response.success) throw new Error('Failed to fetch prompt preview')
      const data = response.data
  ...
  const runAiTest = async () => {
    ...
    try {
      ...
      if (!response.success) throw new Error('Failed to run AI test')
      const data = response.data
    ...
    <DeepVoidBackground className="h-[calc(100vh-64px)] flex flex-col bg-[var(--background)] relative overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {(!isMobile || mobilePanel === 'list') && (
          <div
            className={`${isMobile ? 'flex-1' : 'w-48 flex-shrink-0'} border-r border-[var(--panel-border)] overflow-y-auto bg-[var(--surface-primary)] backdrop-blur-sm z-10`}
          >
            <div className="p-2">
              <div className="flex items-center justify-between mb-2 px-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <label
                    className="p-1 rounded hover:bg-[var(--glass-border)] transition-colors cursor-pointer text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
        ...
        {(!isMobile || mobilePanel === 'preview') && (
          <div
            className={`${isMobile ? 'flex-1' : 'w-[420px] flex-shrink-0'} flex flex-col overflow-hidden`}
          >
            {/* Tabs */}

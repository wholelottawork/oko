...
export const api = {
  ...
  async getWalletBalances(address: string): Promise<WalletBalancesResponse> {
    ...
    if (!result.success) throw new Error(result.message || 'Failed to fetch wallet balances')
    return result.data ?? { assets: [], totalBalanceUsd: '0' }
  },

  async getSolanaTokenBalance(address: string, mint: string): Promise<SolanaTokenBalanceResponse> {
    ...
    if (!result.success) throw new Error(result.message || 'Failed to fetch Solana token balance')
  ...
  async postGeneralChat(
    ...
    const url = lang ? `${API_BASE}/chat?lang=${encodeURIComponent(lang)}` : `${API_BASE}/chat`
    const result = await httpClient.post<{ reply: string; swapIntent?: WalletChatSwapIntent }>(url, body)
    if (!result.success) throw new Error(result.message || 'Chat request failed')
  ...
  createDebateStream(debateId: string): EventSource {
    const token = localStorage.getItem('auth_token')
    return new EventSource(apiUrl(`${API_BASE}/debates/${debateId}/stream?token=${token}`))
  },

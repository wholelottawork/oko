...
export function StrategyStudioPage() {
  ...
  const [mobilePanel, setMobilePanel] = useState<'list' | 'config' | 'preview'>('list')
      ...
      <div className="flex items-center justify-center min-h-[70vh]">
        <div className="text-center">
          <div className="relative">
            <div className="w-16 h-16 rounded-full border-4 strategy-studio-spinner animate-spin" />
            <Zap className="w-6 h-6 text-[var(--text-secondary)] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
          </div>
        </div>
      </div>
    )
  ...
  return (
    <DeepVoidBackground className="h-[calc(100vh-64px)] flex flex-col bg-[var(--background)] relative overflow-hidden">
      ...
      {/* Header */}
      {/* Header */}
      <div className="flex-shrink-0 px-4 py-3 border-b border-[var(--panel-border)] bg-[var(--header-bg)] backdrop-blur-md z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="text-lg font-bold text-[var(--text-primary)]">{t('strategyStudio')}</h1>
              <p className="text-xs text-[var(--text-secondary)]">{t('subtitle')}</p>
            </div>
          </div>
          {error && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs bg-[var(--surface-secondary)] border border-[var(--panel-border)] text-[var(--text-primary)]">
              {error}
      ...
      {isMobile && (
        <div className="flex-shrink-0 flex border-b border-[var(--panel-border)] bg-[var(--surface-primary)]">
          {([
            { id: 'list' as const, label: language === 'zh' ? '策略' : 'Strategies', icon: FileText },
            { id: 'config' as const, label: language === 'zh' ? '配置' : 'Config', icon: Settings },
            { id: 'preview' as const, label: language === 'zh' ? '预览' : 'Preview', icon: Eye },
          ]).map(tab => (
            <button
              key={tab.id}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {(!isMobile || mobilePanel === 'list') && (
          <div className={`${isMobile ? 'flex-1' : 'w-48 flex-shrink-0'} border-r border-[var(--panel-border)] overflow-y-auto bg-[var(--surface-primary)] backdrop-blur-sm z-10`}>
          <div className="p-2">
            <div className="flex items-center justify-between mb-2 px-2">
              <span className="text-xs font-medium text-[var(--text-secondary)]">{t('strategies')}</span>
              <div className="flex items-center gap-1">
                {/* Import button with hidden file input */}
                <label className="p-1 rounded hover:bg-[var(--glass-border)] transition-colors cursor-pointer text-[var(--text-secondary)] hover:text-[var(--text-primary)]" title={language === 'zh' ? '导入策略' : 'Import Strategy'}>
                ...
                <button
                  ...
                  className="p-1 rounded hover:bg-[var(--glass-border)] transition-colors text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
              ...
              {strategies.map((strategy) => (
                <div
                  ...
                  className={`group px-2 py-2 rounded-lg cursor-pointer transition-all ${selectedStrategy?.id === strategy.id
                    ? 'bg-[var(--surface-secondary)] border border-[var(--panel-border)]'
                    : 'hover:bg-[var(--surface-secondary)] border border-transparent'
                    }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm truncate text-[var(--text-primary)]">{strategy.name}</span>
                      ...
                      {!strategy.is_default && (
                        <>
                          ...
                          <button
                            ...
                            className="p-1 rounded hover:bg-[var(--surface-tertiary)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                  ...
                  <div className="flex items-center gap-1 mt-1 flex-wrap">
                    {strategy.is_active && (
                      <span className="px-1.5 py-0.5 text-[10px] rounded border border-[var(--panel-border)] text-[var(--text-secondary)]">
                        {t('active')}
                      </span>
                    )}
                    {strategy.is_default && (
                    ...
                    {strategy.is_public && (
                      <span className="px-1.5 py-0.5 text-[10px] rounded flex items-center gap-0.5 border border-[var(--panel-border)] text-[var(--text-secondary)]">
        ...
        <div className={`${isMobile ? 'flex-1' : 'flex-1 min-w-0'} overflow-y-auto ${isMobile ? '' : 'border-r border-[var(--panel-border)]'}`}>
          {selectedStrategy && editingConfig ? (
            <div className="p-4">
              ...
              <div className={`flex ${isMobile ? 'flex-col gap-3' : 'items-center justify-between'} mb-4`}>
                <div className="flex-1 min-w-0">
                  ...
                  />
                  {hasChanges && (
                    <span className="text-xs text-[var(--text-secondary)]">● {language === 'zh' ? '未保存' : 'Unsaved'}</span>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {!selectedStrategy.is_active && (
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs transition-colors bg-[var(--surface-secondary)] border border-[var(--panel-border)] text-[var(--text-primary)] hover:bg-[var(--surface-tertiary)]"
                    >
                  ...
                  {!selectedStrategy.is_default && (
                    <button
                      ...
                      className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-50
                        ${hasChanges ? 'bg-[var(--accent-primary)] text-[var(--strategy-studio-btn-text)] hover:opacity-90' : 'bg-[var(--surface-secondary)] text-[var(--text-secondary)] cursor-not-allowed'}`}
                    >
              ...
              {editingConfig && (
                <div className="mb-4 p-4 rounded-lg bg-[var(--surface-secondary)] border border-[var(--panel-border)]">
                  <div className="flex items-center gap-2 mb-3">
                    <Zap className="w-4 h-4 text-[var(--text-secondary)]" />
                    <span className="text-sm font-medium text-[var(--text-primary)]">{t('strategyType')}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`p-3 rounded-lg border transition-all ${
                        (!editingConfig.strategy_type || editingConfig.strategy_type === 'ai_trading')
                          ? 'border-[var(--panel-border)] bg-[var(--surface-tertiary)]'
                          : 'border-[var(--panel-border)] hover:bg-[var(--surface-tertiary)]'
                      }`}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <Bot className="w-4 h-4 text-[var(--text-secondary)]" />
                        <span className="text-sm font-medium text-[var(--text-primary)]">{t('aiTrading')}</span>
                      </div>
                      <p className="text-xs text-[var(--text-secondary)] text-left">{t('aiTradingDesc')}</p>
                    </button>
                    <button
                      onClick={() => {
                        if (!selectedStrategy?.is_default) {
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm font-medium text-[var(--text-primary)]">{t('gridTrading')}</span>
                      </div>
                      <p className="text-xs text-[var(--text-secondary)] text-left">{t('gridTradingDesc')}</p>
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <Activity className="w-12 h-12 mx-auto mb-2 opacity-30 text-[var(--text-secondary)]" />
        ...
        <div className={`${isMobile ? 'flex-1' : 'w-[420px] flex-shrink-0'} flex flex-col overflow-hidden`}>
          {/* Tabs */}
          <div className="flex-shrink-0 flex border-b border-[var(--panel-border)]">
            <button
              onClick={() => setActiveRightTab('prompt')}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${activeRightTab === 'prompt' ? 'border-[var(--text-secondary)] text-[var(--text-primary)]' : 'border-transparent opacity-60 hover:opacity-100 text-[var(--text-secondary)]'
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${activeRightTab === 'test' ? 'border-[var(--text-secondary)] text-[var(--text-primary)]' : 'border-transparent opacity-60 hover:opacity-100 text-[var(--text-secondary)]'
                }`}
            >
            ...
            {activeRightTab === 'prompt' ? (
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <select
                    ...
                    className="px-2 py-1.5 rounded text-xs bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors disabled:opacity-50 bg-[var(--accent-primary)] text-[var(--strategy-studio-btn-text)] hover:opacity-90"
                  >
                ...
                {promptPreview ? (
                  <>
                    {/* Config Summary */}
                    <div className="p-2 rounded-lg bg-[var(--surface-primary)] border border-[var(--panel-border)]">
                      <div className="flex items-center gap-1.5 mb-2">
                        <Code className="w-3 h-3 text-[var(--text-secondary)]" />
                        <span className="text-xs font-medium text-[var(--text-secondary)]">Config</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        {Object.entries(promptPreview.config_summary || {}).map(([key, value]) => (
                          <div key={key}>
                            <div className="text-[var(--text-secondary)]">{key.replace(/_/g, ' ')}</div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs font-medium text-[var(--text-primary)]">{t('systemPrompt')}</span>
                        </div>
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--surface-secondary)] text-[var(--text-secondary)]">
                          {promptPreview.system_prompt.length.toLocaleString()} chars
                        </span>
                      </div>
                      <pre
                        className="p-2 rounded-lg text-[11px] font-mono overflow-auto bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <Bot className="w-4 h-4 text-[var(--text-secondary)]" />
                    <span className="text-xs font-medium text-[var(--text-primary)]">{t('selectModel')}</span>
                  </div>
                  {aiModels.length > 0 ? (
                    <select
                      value={selectedModelId}
                      onChange={(e) => setSelectedModelId(e.target.value)}
                      className="w-full px-3 py-2 rounded-lg text-sm bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"
                    >
                      {aiModels.map((model) => (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-50 bg-[var(--accent-primary)] text-[var(--strategy-studio-btn-text)] hover:opacity-90"
                    >
                      {isRunningAiTest ? (
                        <>
                  ...
                  <p className="text-[10px] text-[var(--text-secondary)]">{t('testNote')}</p>
                </div>
                ...
                {aiTestResult ? (
                  <div className="space-y-3">
                    {aiTestResult.error ? (
                      <div className="p-3 rounded-lg bg-[var(--surface-secondary)] border border-[var(--panel-border)]">
                        <p className="text-sm text-[var(--text-primary)]">{aiTestResult.error}</p>
                      </div>
                    ) : (
                      <>
                        ...
                        {aiTestResult.user_prompt && (
                          <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                              <Terminal className="w-3 h-3 text-[var(--text-secondary)]" />
                              <span className="text-xs font-medium text-[var(--text-primary)]">{t('userPrompt')} (Input)</span>
                            </div>
                            <pre
                              className="p-2 rounded-lg text-[10px] font-mono overflow-auto bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"
                        ...
                        {aiTestResult.reasoning && (
                          <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                              ...
                              <span className="text-xs font-medium text-[var(--text-primary)]">{t('reasoning')}</span>
                            </div>
                            <pre
                              className="p-2 rounded-lg text-[10px] font-mono overflow-auto whitespace-pre-wrap bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"
                              style={{ maxHeight: '200px' }}
                            >
                              {aiTestResult.reasoning}
                            </pre>
                        ...
                        {aiTestResult.decisions && aiTestResult.decisions.length > 0 && (
                          <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                              ...
                              <span className="text-xs font-medium text-[var(--text-primary)]">{t('decisions')}</span>
                            </div>
                            <pre
                              className="p-2 rounded-lg text-[10px] font-mono overflow-auto bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"
                              style={{ maxHeight: '200px' }}
                        ...
                        {aiTestResult.ai_response && (
                          <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                              <FileText className="w-3 h-3 text-[var(--text-secondary)]" />
                              <span className="text-xs font-medium text-[var(--text-primary)]">{t('aiOutput')} (Raw)</span>
                            </div>
                            <pre
                              className="p-2 rounded-lg text-[10px] font-mono overflow-auto whitespace-pre-wrap bg-[var(--surface-primary)] border border-[var(--panel-border)] text-[var(--text-primary)]"

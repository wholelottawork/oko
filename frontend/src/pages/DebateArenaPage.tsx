...
const PERS: Record<DebatePersonality, { Icon: React.ComponentType<{ size?: number; className?: string }>; name: string; nameEn: string }> = {
  bull: { Icon: TrendingUp, name: '多头', nameEn: 'Bull' },
  bear: { Icon: TrendingDown, name: '空头', nameEn: 'Bear' },
  analyst: { Icon: BarChart2, name: '分析', nameEn: 'Analyst' },
  contrarian: { Icon: RefreshCcw, name: '逆势', nameEn: 'Contrarian' },
  risk_manager: { Icon: Shield, name: '风控', nameEn: 'Risk Mgr' },
}
...
function PersonalitySelect({ value, onChange, language }: { value: DebatePersonality; onChange: (p: DebatePersonality) => void; language: string }) {
  const [open, setOpen] = useState(false)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-[var(--surface-secondary)] shadow-xl rounded-xl w-full max-w-2xl min-w-[480px] p-8 debate-modal">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-[var(--text-primary)]">{t('createDebate', language)}</h3>
        ...
        <div className="flex flex-col gap-3">
          ...
          <input
            ...
            placeholder={language === 'zh' ? '交易对 (如 BTCUSDT，留空自动选择)' : 'Trading pair (e.g. BTCUSDT, empty = auto)'}
            className="w-full block px-3 py-2 rounded-lg bg-[var(--surface-primary)] debate-input text-[var(--text-primary)] text-sm"
          />
          ...
          {isStaticWithCoins && (
            <select
              ...
              className="w-full block px-3 py-2 rounded-lg bg-[var(--surface-primary)] debate-input text-[var(--text-primary)] text-sm"
              title={language === 'zh' ? '从策略币种快速选择' : 'Quick select from strategy'}
            >
              <option value="">{language === 'zh' ? '选...' : 'Pick...'}</option>
  ...
  return (
    <DeepVoidBackground className="flex overflow-hidden relative" style={{ height: 'calc(100vh - 64px)' }} contentDirection="row" disableAnimation>
      ...
      <div className="w-56 flex-shrink-0 bg-[var(--surface-primary)]/80 backdrop-blur-md border-r debate-panel-border flex flex-col z-10">
        ...
        {/* Debate List */}
        <div className="px-2 py-1 text-xs text-[var(--text-secondary)] font-semibold">{t('debateSessions', language)}</div>
            ...
            {traders?.filter(tr => !tr.is_running).slice(0, 3).map(tr => (
              <div key={tr.trader_id} className="p-2 rounded-lg bg-[var(--surface-secondary)] opacity-50">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-[var(--text-primary)] font-medium truncate">{tr.trader_name}</div>
                    <div className="text-xs text-[var(--text-secondary)]">{t('offline', language)}</div>
                  </div>
                </div>
              </div>
            ))}
            {(!traders || traders.length === 0) && (
              <div className="text-xs text-[var(--text-secondary)] text-center py-4">{t('noTraders', language)}</div>
            )}
          </div>
        </div>
      </div>
              ...
              {Object.keys(rounds).length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-[var(--text-secondary)]">
                  ...
                  <div className="text-lg text-center max-w-md">
                    {detail.status === 'pending' ? t('clickToStart', language) : detail.status === 'completed' ? t('debateFailed', language) : t('waitingAI', language)}
                  </div>
                </div>
              ) : (
                <>
            ...
            {(decision || votes.length > 0) && (
              <div className="p-3 border-t debate-panel-border debate-consensus-bar backdrop-blur-md flex items-center gap-4 flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[var(--text-secondary)]">{t('consensus', language)}:</span>
                  {decision ? (
                    <>
                ...
                {decision && (
                  <div className="flex items-center gap-4 text-sm">
                    <span><span className="text-[var(--text-secondary)]">{t('confidence', language)}</span> <span className="debate-accent font-bold">{decision.confidence || 0}%</span></span>
                    {(decision.leverage ?? 0) > 0 && <span><span className="text-[var(--text-secondary)]">{t('leverage', language)}</span> <span className="text-[var(--text-primary)] font-bold">{decision.leverage}x</span></span>}
                    {(decision.position_pct ?? 0) > 0 && <span><span className="text-[var(--text-secondary)]">{t('position', language)}</span> <span className="text-[var(--text-primary)] font-bold">{((decision.position_pct ?? 0) * 100).toFixed(0)}%</span></span>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-[var(--surface-secondary)]/90 backdrop-blur-xl rounded-xl w-full max-w-sm p-6 debate-modal">
            <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4 flex items-center gap-2">
              <Zap className="debate-accent" /> {t('executeTitle', language)}
            </h3>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setExecId(null); setTraderId('') }}
                className="flex-1 py-2 rounded-lg bg-[var(--surface-primary)] debate-input text-[var(--text-primary)] text-sm hover:bg-[var(--surface-tertiary)] transition-colors">{t('cancel', language)}</button>

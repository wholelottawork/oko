...
export function SwapCard({ intent, address, language = 'en', destAddressOverride }: SwapCardProps) {
  ...
  const fetchQuote = useCallback(async () => {
    ...
    try {
      ...
      if (!from || !to) { setResolveError(language === 'zh' ? '无法解析代币' : 'Could not resolve tokens'); setState('error'); return }
  ...
  const executeSwap = useCallback(async () => {
    ...
    if (isNaN(chainId)) { setError(language === 'zh' ? '暂不支持该链' : 'Chain not supported'); setState('error'); return }
    try {
  ...
  // ── Error state ───────────────────────────────────────────────
  if (state === 'error') {
    const isCancelled = (resolveError || error || '').toLowerCase() === 'cancelled' || (resolveError || error || '').toLowerCase() === '已取消'
    return (
      <div style={cardStyle} className="p-5">
        <div className="flex flex-col items-center gap-3 py-2">
          <div
        ...
        {needsDestAddress && (
          <div className="space-y-1.5">
            <div className="flex items-center gap-1.5 text-[10px]" style={{ color: '#B0B8BB' }}>
              ...
              {language === 'zh' ? '跨链兑换：请输入目标链的接收地址' : 'Cross-chain swap: enter your destination address'}
            </div>
            <input
              type="text"
              value={destAddress}
              onChange={(e) => setDestAddress(e.target.value)}
              placeholder={language === 'zh' ? '目标地址 (0x...)' : 'Destination address (0x...)'}

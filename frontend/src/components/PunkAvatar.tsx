...
const ACCESSORY_COLORS = [
  'var(--accent-primary)', '#0ECB81', '#F6465D', 'var(--accent-primary-hover)', '#a855f7',
...
export function PunkAvatar({ seed, size = 40, className = '' }: PunkAvatarProps) {
  ...
  const renderHair = () => {
    ...
    switch (hairStyle) {
      ...
      case 7: // Crown
        return (
          <>
            <rect x={7*px} y={4*px} width={10*px} height={1*px} fill="var(--accent-primary)" />
            <rect x={8*px} y={2*px} width={2*px} height={2*px} fill="var(--accent-primary)" />
            <rect x={11*px} y={1*px} width={2*px} height={3*px} fill="var(--accent-primary)" />
            <rect x={14*px} y={2*px} width={2*px} height={2*px} fill="var(--accent-primary)" />
          </>
        )
      default:
        return null
  ...
  const renderAccessories = () => {
    ...
    if (hasEarring) {
      elements.push(
        <circle key="earring" cx={5*px} cy={12*px} r={px} fill="var(--accent-primary)" />
      )
    }

...
export function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (betaMode && !betaCode.trim()) {
      setError(language === 'zh' ? '内测期间，注册需要提供内测码' : 'Beta code required during closed beta')
      return
    }
                ...
                {betaMode && (
                  <div>
                    ...
                    <p className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>
                      {language === 'zh' ? '内测期间需要' : 'Required during closed beta'}
                    </p>
                  </div>
                )}
            ...
            {step === 'setup-otp' && (
              <div className="space-y-6">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="font-medium mb-0.5" style={{ color: 'var(--text-primary)' }}>
                        {language === 'zh' ? '安装验证器应用' : 'Install authenticator app'}
                      </p>
                      <p className="text-xs">
                        {language === 'zh' ? '推荐：Google Authenticator' : 'Recommended: Google Authenticator'}
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs">
                        {language === 'zh' ? '打开验证器应用，扫描上方二维码' : 'Open your app and scan the code above'}
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs">
                        {language === 'zh' ? '在下一步输入应用生成的6位验证码' : 'Enter the 6-digit code in the next step'}
                      </p>
                      <p className="text-xs mt-2 italic" style={{ color: 'var(--text-tertiary)' }}>
                        {language === 'zh' ? '提示：确保手机时间设置为自动同步' : 'Tip: Ensure your phone\'s time is set to automatic'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  style={btnPrimaryStyle}
                >
                  {language === 'zh' ? '我已扫描，继续' : 'I have scanned the code'}
                </button>
              </div>
            )}
              ...
              <form onSubmit={handleOTPVerify} className="space-y-6">
                ...
                >
                  {loading
                    ...
                    : (language === 'zh' ? '激活账户' : 'Activate account')}
                </button>
              </form>
            )}
          </div>

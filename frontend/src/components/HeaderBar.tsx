...
export default function HeaderBar({
      ...
      items: [
        {
          page: 'trader',
          path: '/dashboard',
          label: isEn ? 'Dashboard' : '仪表盘',
          description: isEn ? 'AI trader performance overview' : 'AI 交易员表现概览',
          requiresAuth: true,
        },
        {
          page: 'traders',
          path: '/traders',
          label: isEn ? 'Traders' : '交易员',
          description: isEn ? 'Configure and manage trading agents' : '配置和管理 AI 交易代理',
          requiresAuth: true,
        },
        {
          page: 'strategy',
          path: '/strategy',
          label: isEn ? 'Strategies' : '策略',
          description: isEn ? 'Build & manage trading strategies' : '创建和管理交易策略',
          requiresAuth: true,
        },
        {
          page: 'strategy-market',
          path: '/strategy-market',
          label: isEn ? 'Strategy Market' : '策略市场',
          description: isEn ? 'Browse community strategies' : '浏览社区策略',
          requiresAuth: true,
        },
        {
          page: 'competition',
          path: '/competition',
          label: isEn ? 'Live Competition' : '实时排行',
          description: isEn ? 'Real-time AI trader leaderboard' : '实时 AI 交易员排行榜',
          requiresAuth: true,
        },
        {
          page: 'debate',
          path: '/debate',
          label: isEn ? 'Debate' : '辩论',
          description: isEn ? 'Watch AI traders debate market moves live' : '观看 AI 交易员实时辩论市场走势',
          requiresAuth: true,
        },
        {
          page: 'backtest',
          path: '/backtest',
          label: isEn ? 'Backtest' : '回测',
          description: isEn ? 'Test strategies against historical data' : '针对历史数据测试策略',
          requiresAuth: true,
        },
        {
          page: 'data',
          path: '/data',
          label: isEn ? 'Data' : '数据',
          description: isEn ? 'Market data & analytics' : '市场数据与分析',
          requiresAuth: false,
        },
      ],
    },
  ...
  const isCategoryActive = (cat: NavCategory) =>
    currentPage !== undefined && cat.activePages.includes(currentPage)
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1">
              {OFFICIAL_LINKS.github ? (
                ...
                <span className="p-2 rounded-lg text-nofx-text-muted cursor-default opacity-60" title="GitHub">
                  <svg width="18" height="18" viewBox="0 0 16 16" fill="currentColor">
                    <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" />
                  </svg>
                </span>
              ...
              {OFFICIAL_LINKS.twitter ? (
                ...
                <span className="p-2 rounded-lg text-nofx-text-muted cursor-default opacity-60" title="Twitter">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                  </svg>
                </span>
              ...
              {OFFICIAL_LINKS.telegram ? (
                ...
                <span className="p-2 rounded-lg text-nofx-text-muted cursor-default opacity-60" title="Telegram">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                  </svg>
                </span>
            ...
            {isLoggedIn && user ? (
              <div className="relative" ref={userDropdownRef}>
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded transition-colors bg-nofx-bg-lighter header-border border hover:bg-white/5"
                >
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold header-user-avatar">
                    {user.email[0].toUpperCase()}
                  </div>
                  <span className="text-sm text-nofx-text-muted">{user.email}</span>
                  <ChevronDown className="w-4 h-4 text-nofx-text-muted" />
                </button>

                {userDropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-full rounded-lg shadow-lg overflow-hidden z-50 bg-nofx-bg-lighter header-border border">
                    <div className="px-3 py-2 border-b header-border">
                      <div className="text-xs text-nofx-text-muted">{t('loggedInAs', language)}</div>
                      <div className="text-sm font-medium text-nofx-text-muted">{user.email}</div>
                    </div>
                    {onLogout && (
                      <button

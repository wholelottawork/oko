...
### What is OKO?
OKO is an AI-powered cryptocurrency trading bot that uses large language models (LLMs) to make trading decisions on futures markets.
...
Yes! OKO supports running multiple traders with different configurations, AI models, and trading strategies.
...
No! OKO has a web UI for all configuration. However, basic command line knowledge helps with setup and troubleshooting.
...
**Recommended**: Yes, use a subaccount dedicated to OKO for better risk isolation. However, note that some subaccounts have restrictions (e.g., 5x max leverage on Binance).
...
**Solution**: Switch to **Hedge Mode** (dual-side positions)
...
**Why**: OKO uses `PositionSide(LONG/SHORT)` which requires Hedge Mode.

See [Issue #202](https://github.com/oko-trading/okotrading/issues/202) and [Troubleshooting Guide](TROUBLESHOOTING.md#-only-opening-short-positions-issue-202).
...
Yes, to some extent. OKO provides historical performance feedback in each decision prompt, allowing the AI to adjust its strategy.
...
docker compose exec postgres \
  psql -U oko -d oko -c "SELECT * FROM trades;"
```
...
2. Search [existing issues](https://github.com/oko-trading/okotrading/issues)
...
Open a [Feature Request](https://github.com/oko-trading/okotrading/issues/new/choose) with your idea!

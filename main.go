...
func main() {
	// Load .env from cwd and next to the binary (PM2 / systemd often have a different cwd)
	loadDotEnv()
	...
	// Start live price ticker service (single Binance WebSocket, fans out via SSE)
	go market.StartTickerService()
	logger.Info("📈 Live ticker service starting...")

	// Start market data cache service (global stats, top coins, trending, gainers)
	go market.StartMarketDataService()
...
func newSharedMCPClient() mcp.AIClient {
	...
	client.(*mcp.DeepSeekClient).SetAPIKey(apiKey, "", "")
	return client
}

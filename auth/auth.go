...
// Uses issuer param instead of "Issuer:Account" in label so authenticator apps show "Issuer (Account)" not "Issuer: Account".
func GetOTPQRCodeURL(secret, email string) string {
	return fmt.Sprintf("otpauth://totp/%s?secret=%s&issuer=%s", url.PathEscape(email), secret, OTPIssuer)
}

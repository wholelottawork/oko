...
type CoinSourceConfig struct {
	...
	// Whether to use the OI-increase ranking, which can surface long candidates.
	UseOITop bool `json:"use_oi_top"`
	// OI Top maximum count
	OITopLimit int `json:"oi_top_limit,omitempty"`
	// Whether to use the OI-decrease ranking, which can surface short candidates.
...
func (s *StrategyStore) initDefaultData() error {
	var count int64
	if err := s.db.Model(&Strategy{}).Where("is_default = ?", true).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
		return nil
	...
	st := &Strategy{
		ID:          "default-strategy",
		UserID:      "default",
		Name:        "Default AI Trading",
		Description: "Built-in strategy for AI trading and debate",
		IsActive:    false,
		IsDefault:   true,
		Config:      string(configJSON),
	}
	return s.db.FirstOrCreate(st, "id = ?", st.ID).Error
}

...
func (e *DebateEngine) InitializeClients(participants []*store.DebateParticipant) error {
	...
	for _, p := range participants {
		...
		// API key: fall back to system .env key when user has not configured their own
		apiKey := strings.TrimSpace(string(aiModel.APIKey))
		if apiKey == "" {
			apiKey = config.Get().GetSystemAPIKey(aiModel.Provider)
		}
		// Model name: use server's .env when user has not set a valid Custom Model Name
		// (treat bare provider names like "deepseek"/"claude" as invalid - they cause "Model Not Exist")
		modelName := strings.TrimSpace(aiModel.CustomModelName)
		if modelName == "" || modelName == aiModel.Provider {
			modelName = config.Get().GetSystemModelName(aiModel.Provider)
		}
		client.SetAPIKey(apiKey, aiModel.CustomAPIURL, modelName)
